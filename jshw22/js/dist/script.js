...
(function ($) {
	$(function () {
		var obj = [{ title: '1. Вопрос №1',
			answer: [{ text: 'Вариант ответа НЕТ', correct: false }, { text: 'Вариант ответа НЕТ', correct: false }, { text: 'Вариант ответа ДА', correct: true }]
		}, { title: '2. Вопрос №2',
			answer: [{ text: 'Вариант ответа НЕТ', correct: false }, { text: 'Вариант ответа НЕТ', correct: false }, { text: 'Вариант ответа ДА', correct: true }]
		}, { title: '3. Вопрос №3',
			answer: [{ text: 'Вариант ответа НЕТ', correct: false }, { text: 'Вариант ответа НЕТ', correct: false }, { text: 'Вариант ответа ДА', correct: true }]
		...
		/*localStorage.clear();*/
		try {
			if (localStorage.getItem('dan') === null) {
				localStorage.getItem('dan');
			};
		...
		$('#but1').click(function (event) {
			...
			$question.each(function (i, elem) {
				...
				if (check.length === 1 && $(elem).children(".true").find('input:checkbox').prop("checked")) {
					$result++;
				};
			});
