...
var data_array_new =JSON.stringify(data_array);
var data_array = JSON.parse(data_array_new);
...
/*_.map выбор массива skills
_.reduceRight объединение подмассивов в один
_.uniqBy только уникальные значения
_.transform изменение элементов массива на элементы с символами в нижнем регистре с помощью _.lowerCase
_.sortBy сортировка
*/
var skills=_.sortBy(
										_.transform( 
																_.uniqBy(
																					_.reduceRight(
																												...
																												, function(flattened, other) { return flattened.concat(other);}, []
																												)
																				)
																, function(result, n) {result.push(n = _.lowerCase(n));return n;}, []
																)
										);
console.log('Массив скиллов (поле skills) всех людей, не должно быть повторяющихся скиллов, так же они должны быть отсортированы по алфавиту: ',skills);

 /*2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends); */
 /*_.map выбор имен в массив*/
	var name1=_.map(data_array, 'name');
	/*_.map выбор друзей в массив*/
	var friends=_.map(data_array, 'friends');
	/*создание массива кол-во друзей*/
	var array_kolvo_friends = [];
	_.forEach(friends, function(value, key) {
	array_kolvo_friends =_.concat(array_kolvo_friends,_.size(value));
	});
	/*_.zip объединение этих массивов в подмассивы - [имя,кол-во друзей]*/
	var zipped = _.zip(name1, array_kolvo_friends);
	/*_.orderBy сортировка подмассивов по элементам*/
	var sort=_.orderBy(zipped, ['1', '0'], ['desc','asc']);
	/*Выбор из массива только отсортированных имен*/
	console.log('Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends): ', _.map(sort, '0')); 


 /*3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей*/
  var array_name_friends = [];
 _.forEach(friends, function(value, key) {
		array_name_friends =_.concat(array_name_friends,_.map(value,'name'));
	})	;
	var unic_friends=_.sortBy(_.uniq(array_name_friends));
	console.log('Массив всех друзей всех пользователей, не должно быть повторяющихся людей: '+[unic_friends]);